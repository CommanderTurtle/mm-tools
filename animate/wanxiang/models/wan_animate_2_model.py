...
class Incontext_AttentionBlock(nn.Module):
    ...
    def forward_ref(self, x_ref, index, k_cache, v_cache, context_ref, freqs_ref, grid_sizes_ref,  e_ref, context_lens):
        ...
        if self.use_context_parallel:
            ...
            qkv_ref = ops.diff_all_to_all(qkv_ref, scatter_dim=2, gather_dim=1, group=ops.get_tensor_parallel_group())
        ...
        if self.use_context_parallel:
            xout_ref = ops.diff_all_to_all(xout_ref, scatter_dim=1, gather_dim=2, group=ops.get_tensor_parallel_group())
    ...
    def forward_gen(self, x, index, k_cache, v_cache, block_mask, context, freqs, freqs_ref, grid_sizes, grid_sizes_ref, 
        ...
        if self.use_context_parallel:
            ...
            qkv = ops.diff_all_to_all(qkv, scatter_dim=2, gather_dim=1, group=ops.get_tensor_parallel_group())
        ...
        if self.use_context_parallel:
            xout = ops.diff_all_to_all(xout, scatter_dim=1, gather_dim=2, group=ops.get_tensor_parallel_group())
...
class WanAnimate2Transformer(nn.Module):
    ...
    def __init__(
        ...
        self.blocks = nn.ModuleList([Incontext_AttentionBlock(
           ...
           sparse_type=self.sparse_type, log_scale=self.log_scale,
    ...
    def forward_ref(
        ...
        if self.use_context_parallel:
            sp_group = ops.get_tensor_parallel_group()

            x_ref = ops.split_forward_gather_backward(
                x_ref, dim=1, group=sp_group, grad_scale="down"
            )
    ...
    def forward_gen(
        ...
        if self.use_context_parallel:
            sp_group = ops.get_tensor_parallel_group()
            x = ops.split_forward_gather_backward(
                x, dim=1, group=sp_group, grad_scale="down"
            )

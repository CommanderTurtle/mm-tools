...
class MoGeModel(nn.Module):
    ...
    def from_pretrained(cls, pretrained_model_name_or_path: Union[str, Path, IO[bytes]], model_kwargs: Optional[Dict[str, Any]] = None, **hf_kwargs) -> 'MoGeModel':
        ...
        ### Parameters:
        - `pretrained_model_name_or_path`: path to the project-local checkpoint file.
        ...
        if not checkpoint_path.is_file():
            raise FileNotFoundError(f"Missing project-local MoGe checkpoint: {checkpoint_path}")
    ...
    def init_weights(self):
        raise RuntimeError("Initialize MoGe from a project-local checkpoint")

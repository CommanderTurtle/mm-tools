...
Exported from the studio adapter in `./local_app/adapter.py` (the graphs
...
provide the GPU-only loaders (`./ComfyUI-MMToolsAnimate`:
`MMToolsGpuOnlyWanLoader`, `MMToolsGpuOnlyWan22Loader`) and the ONNX pose
pipeline (`./ComfyUI-WanAnimatePreprocess`: `OnnxDetectionModelLoader`,

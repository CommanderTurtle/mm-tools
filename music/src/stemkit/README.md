...
*   **Fully offline builds:** if you'd rather send nothing, build from source and remove `src/main/telemetry.ts` (or block the worker domain in your firewall) — everything else works identically offline. *This vendored copy has `src/main/telemetry.ts` removed and no call sites remain, so it never pings.*
...
## mm-tools vendor notes

Vendored into mm-tools as `music/src/stemkit` from
[danielravina/stemkit](https://github.com/danielravina/stemkit) at commit
`2d44bc006e8f98170403ec131353aa7f66b49540` (v0.1.22, 2026-09-18); the upstream
history is preserved in `.git-archives/stemkit.git.tar`. Copyright (c) 2026
daniel ravina; MIT licensed. Nested `.git`, `.gitignore`, `.nvmrc` and the
top-level `LICENSE` file were dropped per mm-tools convention; the MIT terms
and attribution above are the preserved notice. `src/main/telemetry.ts` and
its call sites were removed so the anonymous install ping can never fire.
The runtime surface used by the mm-tools studios is `python/separate.py`
(demucs) and `python/roformer.py` (Mel-Band Roformer vocals), driven as
subprocesses with the JSON progress protocol they already emit.

...
class ComfyRuntime:
    ...
    Spawns a repository-pinned Comfy tree under a private subprocess so its
    quantized models get their intended loader and scheduler without exposing
    Comfy's editor or API to the LAN. The outer Studio remains the sole
    product/API surface and owns queueing/cancellation.
    """
    ...
    def __init__(
        ...
        *,
        ...
        extra_model_paths: dict[str, dict[str, str]] | None = None,
    ) -> None:
        self.python = python
        ...
        self.temp_dir = runtime_root / "comfy-temp" / context.job_id
        self.log_path = runtime_root / "comfy-logs" / f"{context.job_id}.log"
        for directory in (self.input_dir, self.output_dir, self.user_dir, self.temp_dir, self.log_path.parent):
            directory.mkdir(parents=True, exist_ok=True)
        self.model_paths_config = self._write_model_paths(extra_model_paths, self.user_dir)
        env = os.environ.copy()
        env.update({
        ...
        command = [
            ...
            "--database-url", f"sqlite:///{self.user_dir / 'comfyui.db'}",
        ...
        if self.model_paths_config is not None:
            command.extend(["--extra-model-paths-config", str(self.model_paths_config)])
    ...
    @staticmethod
    def _write_model_paths(extra_model_paths: dict[str, dict[str, str]] | None, user_dir: Path) -> Path | None:
        """Write the loopback-only extra model path map as YAML.

        JSON string quoting is a valid YAML scalar, so the writer needs no
        PyYAML dependency in the calling studio's environment.
        """
        if not extra_model_paths:
            return None
        lines: list[str] = []
        for group, paths in extra_model_paths.items():
            lines.append(f"{group}:")
            for key, value in paths.items():
                lines.append(f"  {key}: {json.dumps(str(value))}")
        config = user_dir / "extra_model_paths.yaml"
        config.write_text("\n".join(lines) + "\n", encoding="utf-8")
        return config
    ...
    def cleanup(self) -> None:
        self.close()
        for directory in (self.input_dir, self.output_dir, self.user_dir, self.temp_dir):
